...
public class Homework6Test {
    ...
    public static  void initTable(HashTableChained table, int numBoards, SimpleBoard[] boards) {
	table.makeEmpty();
	for (int i = 0; i < numBoards; i++) {
	    SimpleBoard randomBoard = randomBoard();
	...
	SimpleBoard[] boards = new SimpleBoard[numBoards]; 
	HashTableChained table = new HashTableChained(numBoards);
	initTable(table, numBoards, boards);
	...
	SimpleBoard boardToRemove = boards[73];
	System.out.println("hasCode() "+boardToRemove.hashCode());
	Entry comparer = table.remove(boardToRemove);
	assert boardToRemove.equals(comparer.key());
